...
export const generateSmartPackingList = async (userId, tripId) => {
    ...
    let itenarySummary = "No specific activities planned yet. Suggest general essentials for this destination.";

    if (trip.events && trip.events.length > 0) {
        itenarySummary = trip.events.map(e => `- Activity: "${e.title}" | Vibe: ${e.intensity_level}`).join('\n');
    }//to prevent an empty array coming from .map fxn which evaluates to an empty string(falsy) but may crash google SDK's due to entire text being malformed or missing a required token block
